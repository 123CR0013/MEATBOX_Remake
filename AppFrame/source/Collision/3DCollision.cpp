...
bool Collision3D::OBBCollision(OBB obb_1, OBB obb_2) {
	...
	VECTOR NAe1 = obb_1.vDir[0], Ae1 = VScale(NAe1, obb_1.length[0] / 2);
	VECTOR NAe2 = obb_1.vDir[1], Ae2 = VScale(NAe2, obb_1.length[1]/2);
	VECTOR NAe3 = obb_1.vDir[2], Ae3 = VScale(NAe3, obb_1.length[2]/2);
	VECTOR NBe1 = obb_2.vDir[0], Be1 = VScale(NBe1, obb_2.length[0] / 2);
	VECTOR NBe2 = obb_2.vDir[1], Be2 = VScale(NBe2, obb_2.length[1] / 2);
	VECTOR NBe3 = obb_2.vDir[2], Be3 = VScale(NBe3, obb_2.length[2]/2);
	//Misalignmentでおなかの中心にモデルをずらしている 大体ｙ軸だけで済むと思うがフライト系などの3軸回転を入れるときは
	//VTransformの中の回転行列をy軸だけでなくｘとｚを含めた3軸で回転してください
	VECTOR Interval = VSub(obb_1.centerPos, obb_2.centerPos);

	//  各軸の方向ベクトルと分離軸との絶対値の内積の和をまとめたものと 中心点間の距離を比較し中心点間距離より短かったら衝突している

	// 分離軸 : Ae1
	FLOAT rA = VSize(Ae1);
	FLOAT rB = MyMath::LenSegOnSeparateAxis(NAe1, Be1, Be2, Be3);
	...
	// 分離軸 : Ae2
	rA = VSize(Ae2);
	rB = MyMath::LenSegOnSeparateAxis(NAe2, Be1, Be2, Be3);
	...
	// 分離軸 : Ae3
	rA = VSize(Ae3);
	rB = MyMath::LenSegOnSeparateAxis(NAe3, Be1,Be2,Be3);
	L = fabs(VDot(Interval, NAe3));
	if (L > rA + rB)
		return false;

	// 分離軸 : Be1
	rA = MyMath::LenSegOnSeparateAxis(NBe1, Ae1, Ae2, Ae3);
	rB = VSize(Be1);
	...
	// 分離軸 : Be2
	rA = MyMath::LenSegOnSeparateAxis(NBe2, Ae1, Ae2, Ae3);
	rB = VSize(Be2);
	...
	// 分離軸 : Be3
	rA = MyMath::LenSegOnSeparateAxis(NBe3, Ae1, Ae2, Ae3);
	rB = VSize(Be3);
	...
	rA = MyMath::LenSegOnSeparateAxis(Cross, Ae2, Ae3);
	rB = MyMath::LenSegOnSeparateAxis(Cross, Be2, Be3);
	...
	rA = MyMath::LenSegOnSeparateAxis(Cross, Ae2, Ae3);
	rB = MyMath::LenSegOnSeparateAxis(Cross, Be1, Be3);
	L = fabs(VDot(Interval, Cross));
	if (L > rA + rB)
		return false;
	...
	rA = MyMath::LenSegOnSeparateAxis(Cross, Ae2, Ae3);
	rB = MyMath::LenSegOnSeparateAxis(Cross, Be1, Be2);
	L = fabs(VDot(Interval, Cross));
	if (L > rA + rB)
		return false;
	...
	rA = MyMath::LenSegOnSeparateAxis(Cross, Ae1, Ae3);
	rB = MyMath::LenSegOnSeparateAxis(Cross, Be2, Be3);
	L = fabs(VDot(Interval, Cross));
	if (L > rA + rB)
		return false;
	...
	rA = MyMath::LenSegOnSeparateAxis(Cross, Ae1, Ae2);
	rB = MyMath::LenSegOnSeparateAxis(Cross, Be2, Be3);
	L = fabs(VDot(Interval, Cross));
	if (L > rA + rB)
		return false;
...
POINT_LINE_SHORT Collision3D::PointLineSegShortLength(VECTOR line_start, VECTOR line_end, VECTOR point) {
	...
	if (!MyMath::CheckAcuteAngle(point, line_start, line_end)) {
		//始点側
		Re.hit_point = line_start;
		Re.length = VSize(VSub(line_start, point));
	}
	else if (!MyMath::CheckAcuteAngle(point, VAdd(line_start, end), line_start)) {
		//終点側
...
TWOLINE_SHORT Collision3D::TwoLineShortPoint(VECTOR line_1_start, VECTOR line_1_end, VECTOR line_2_start, VECTOR line_2_end) {
	...
	//平行な直線か調べる
	if (MyMath::CheckParallelRelation(line_1_start, line_1_end, line_2_start, line_2_end)) {
...
TWOLINE_SHORT Collision3D::TwoSegmentShortPoint(VECTOR line_1_start, VECTOR line_1_end, VECTOR line_2_start, VECTOR line_2_end) {
	...
	if (VSquareSize(one_way) < effective_range) {
		...
		if (VSquareSize(two_way) < effective_range) {
			...
			Re.line_2_coefficient = MyMath::Clamp(0, 1.0f, Re.line_2_coefficient);
			return Re;
		}
	}
	//線分2が縮退しているか
		...
		Re.line_2_coefficient = 0.0f;
		Re.length = value.length;
		Re.line_1_coefficient = MyMath::Clamp(0, 1.0f, Re.line_1_coefficient);
		return Re;
	}
	...
	Re.line_1_coefficient = MyMath::Clamp(0, 1, Re.line_1_coefficient);
	Re.line_1_point = VAdd(line_1_start, VScale(one_way, Re.line_1_coefficient));
	...
	//-------------------------------------------------------------------------------------------------------------------------------------------------------
	Re.line_2_coefficient = MyMath::Clamp(0, 1, Re.line_2_coefficient);
	Re.line_2_point = VAdd(line_2_start, VScale(two_way, Re.line_2_coefficient));
...
VECTOR Collision3D::PointOBB(VECTOR point, OBB obb) {
	VECTOR pos = obb.centerPos;
	VECTOR cp = VSub(point, pos);
	VECTOR Re = pos;
	float length = 0.0f;
	for (int i = 0; i < 3; i++) {
		length = VDot(cp, obb.vDir[i]);
		...
		Re = VAdd(Re, VScale(obb.vDir[i], length));
	}
	return Re;
}
...
bool Collision3D::OBBCapsuleCol(OBB obb, VECTOR line_start, VECTOR line_end, float r, VECTOR* hitPos) {
	POINT_LINE_SHORT  a = Collision3D::PointLineSegShortLength(line_start, line_end, obb.centerPos);
