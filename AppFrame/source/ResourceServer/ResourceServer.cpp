...
int ResourceServer::LoadDivGraph(const std::string FileName, int AllNum, int XNum, int YNum, int XSize, int YSize, int* HandleBuf)
{
    return LoadDivGraph(FileName.c_str(), AllNum, XNum, YNum, XSize, YSize, HandleBuf);
}

int ResourceServer::LoadDivGraph(const std::string FileName, int AllNum, int XNum, int YNum, int XSize, int YSize, std::vector<int>& HandleBuf)
{
    // キーの検索
    auto itr = _mapDivGraph.find(FileName);
    if (itr != _mapDivGraph.end())
    {
        ...
        HandleBuf.clear();
        for (int i = 0; i < itr->second.AllNum; i++) {
            HandleBuf.push_back(itr->second.handle[i]);
        }
        return 0;
    }
    // キーが無かった
...
int ResourceServer::LoadSoundMem(const std::string FileName)
{
