...
void MyDraw::MyDrawModiGraph(MATRIX mView, VECTOR position, float zoom, float angle, int width, int height, int cgHandle) {
	// 左右反転しない
	_MyDrawModiGraph(mView, position, zoom, angle, width, height, cgHandle, 0);
}

void MyDraw::MyDrawTurnModiGraph(MATRIX mView, VECTOR position, float zoom, float angle, int width, int height, int cgHandle) {
	// 左右反転する
	_MyDrawModiGraph(mView, position, zoom, angle, width, height, cgHandle, 1);
}

void MyDraw::MyDrawModiGraph(MATRIX mView, VECTOR position, float zoom, float angle, int width, int height, int cgHandle, int turn)
{
...
// 矩形を描く
void MyDraw::MyDrawBox(MATRIX mView, int x1, int y1, int x2, int y2, unsigned int Color, int FillFlag) {
	VECTOR pos[4] = {
		VGet(x1,		y1,		0),	// 左上
		VGet(x2 - 1,	y1,		0),	// 右上
		VGet(x1,		y2 - 1,	0),	// 左下
...
void MyDraw::MyDrawDeformationGraph(MATRIX mView, int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4, float direction, int handle, int FillFlag) {
	VECTOR pos[4] = {
		VGet(x1,		y1,		0),	// 左上
		VGet(x2 - 1,	y2,		0),	// 右上
		VGet(x3,		y3 - 1,	0),	// 左下
...
void MyDraw::MyDrawExtendGraph(MATRIX mView, int x1, int y1, int x2, int y2, unsigned int handle, int FillFlag) {
	VECTOR pos[4] = {
		VGet(x1,		y1,		0),	// 左上
		VGet(x2 - 1,	y1,		0),	// 右上
		VGet(x1,		y2 - 1,	0),	// 左下
...
void MyDraw::MyDrawRect(MATRIX mView, MYRECT_PLUS rc, unsigned int Color, int FillFlag){
	MyDrawBox(mView, rc.x, rc.y, rc.x+rc.w, rc.y+rc.h, Color, FillFlag);
}

void MyDraw::MyDrawLine(MATRIX mView, int x1, int y1, int x2, int y2, int Color ) {
	VECTOR pos[] = {
		VGet(x1,		y1,		0),	// 1
		VGet(x2 - 1,	y2,		0),	// 2
	};
...
void MyDraw::MyDrawCircle(MATRIX mView, int x, int y, int r) {
	MATRIX m = mView;
	VECTOR pos;
	pos = VGet(x, y, 0);
...
void MyDraw::DrawPosString(int x, int y, int posBaseX, unsigned int color, const char* str) {
...
void MyDraw::PortalIn(MATRIX mView, VECTOR position, float zoom, float angle, int width, int height, int GrHandle, int Color, int turn,float param)
{
	// 回転×拡縮×並行移動
	MATRIX mRot = MGetRotZ(angle);						// 2Dでx,yを回転させるには、3DならZ軸回転になる
	MATRIX mScale = MGetScale(VGet(zoom, zoom, 1));		// Z方向の倍率は1のまま
